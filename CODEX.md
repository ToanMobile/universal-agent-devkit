...
# Language Policy: Default communication language: English. Switch to Vietnamese when requested. Code identifiers/paths: always English.

# AGENTS.md — Master Rules & Universal Multi-Agent Architecture

Shared baseline and **Single Source of Truth (SSOT)** for AI coding agents across multiple platforms: **Antigravity (Google / Gemini)**, **Claude Code (Anthropic)**, **Cursor**, **Windsurf**, and **Codex**.
...
3. `CLAUDE.md` and `.claude/` for detailed project contracts, commands, knowledge, and memory.
4. Domain-specific rulebooks and existing code patterns/tests.
...
- **Gate 2 (Strict Narrow Scope):** Applies ONLY when touching:
  ...
  (d) Meta-tooling or rule files (`AGENTS.md`, `CLAUDE.md`, `.claude/rulebook/`, `scripts/qa`).
...
- **Antigravity (Google / Gemini):** Reads `AGENTS.md` and discovers all skills in `.agents/skills/*/SKILL.md`.
- **Claude Code (Anthropic):** Reads `CLAUDE.md` (which imports `@AGENTS.md`), loads `.claude-plugin/plugin.json`, executes slash commands, and enforces safety hooks.
- **Cursor / Windsurf:** Loads `.cursorrules` or `.cursor/rules/*.mdc` generated from this devkit.
- **Synchronization:** Run `./bin/agent-kit sync` anytime new skills, rules, or aliases are modified.
